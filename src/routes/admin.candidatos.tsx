...
type CandidateView = StaffApplication & { profile?: { username: string | null; display_name: string | null; avatar_url: string | null }; progress?: Progress | null };
...
function CandidatesAdminPage() {
  ...
  const load = async () => {
    ...
    const { data: profiles } = userIds.length ? await supabase.from("profiles").select("id, username, display_name, avatar_url").in("id", userIds) : { data: [] };
  ...
  const filtered = useMemo(() => items.filter((item) => {
    const haystack = `${item.profile?.username ?? ""} ${item.profile?.display_name ?? ""} ${item.motivation}`.toLowerCase();
    ...
    <div className="grid gap-6 lg:grid-cols-[380px_minmax(0,1fr)]">
      <div className="space-y-3">
        ...
        {filtered.map((candidate) => <Button key={candidate.id} variant="ghost" onClick={() => void openCandidate(candidate)} className={`glass h-auto w-full justify-start whitespace-normal rounded-lg border p-4 text-left ${selected?.id === candidate.id ? "border-neon-cyan" : "border-border"}`}>
          <UserRound className="h-9 w-9 text-neon-cyan" /><span className="min-w-0 flex-1"><span className="block truncate font-bold">{candidate.profile?.display_name || candidate.profile?.username || "Usuario"}</span><span className="block text-xs text-muted-foreground">{STATUS_META[candidate.status].label} · {candidate.progress?.percent ?? 0}%</span></span><ChevronRight />
        </Button>)}
      </div>
      ...
      {!selected ? <div className="glass rounded-lg border border-border p-10 text-center"><ClipboardCheck className="mx-auto h-12 w-12 text-neon-purple" /><h2 className="mt-4 text-xl font-bold">Selecciona una candidatura</h2><p className="mt-2 text-muted-foreground">Aquí aparecerá la solicitud completa y sus controles.</p></div> : <div className="space-y-5">
        <section className="glass rounded-lg border border-border p-5 sm:p-7">
          <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs uppercase text-neon-cyan">{selected.profile?.username ? `@${selected.profile.username}` : selected.user_id}</p><h1 className="mt-1 text-2xl font-bold">{selected.profile?.display_name || "Candidato"}</h1><p className="text-sm text-muted-foreground">{PHASE_LABEL[selected.phase]}</p></div><span className="rounded-md border border-border px-3 py-1.5 text-xs">{STATUS_META[selected.status].dot} {STATUS_META[selected.status].label}</span></div>

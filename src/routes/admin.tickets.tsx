...
export const Route = createFileRoute("/admin/tickets")({
  head: () => ({ meta: [{ title: "Admin · Tickets — ItsaBDias" }] }),
...
function AdminTicketsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data } = await supabase.from("user_roles").select("role").eq("user_id", user.id).in("role", ["admin", "founder", "moderator"]);
  ...
  return (
    <PageShell>
      <section className="py-14 px-4 sm:px-6">
        <SectionTitle eyebrow="// admin.tickets" title="Gestión de tickets" subtitle="Responde y resuelve tickets de la comunidad." />
        <div className="mx-auto max-w-5xl">
          <div className="mb-6"><Link to="/admin" className="text-xs px-3 py-1.5 rounded-md border border-border hover:border-neon-cyan/60">← Dashboard</Link></div>
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            <button onClick={() => setFilter("all")} className={`px-3 py-1.5 rounded-md text-xs border ${filter === "all" ? "bg-gradient-neon text-primary-foreground border-transparent" : "border-border text-muted-foreground"}`}>Todos ({tickets.length})</button>
            {STATUSES.map((s) => (
              <button key={s.value} onClick={() => setFilter(s.value)} className={`px-3 py-1.5 rounded-md text-xs border ${filter === s.value ? "bg-gradient-neon text-primary-foreground border-transparent" : "border-border text-muted-foreground"}`}>{s.label}</button>
            ))}
          </div>
          <div className="space-y-4">
            {filtered.length === 0 && <p className="text-muted-foreground text-sm">Sin tickets.</p>}
            {filtered.map((t) => {
              const status = STATUSES.find((s) => s.value === t.status) ?? STATUSES[0];
              return (
                <div key={t.id} className="glass rounded-xl p-5 space-y-3">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-xs px-2 py-0.5 rounded-full bg-neon-purple/20 text-neon-purple border border-neon-purple/40">{t.category}</span>
                    <span className={`text-xs px-2 py-0.5 rounded-full border inline-flex items-center gap-1 ${status.color}`}><status.icon className="h-3 w-3" /> {status.label}</span>
                  </div>
                  <h4 className="font-bold text-lg">{t.title}</h4>
                  <p className="text-sm text-muted-foreground whitespace-pre-wrap">{t.body}</p>
                  {t.username && <p className="text-xs text-neon-cyan">por {t.username}</p>}
                  <div className="flex flex-wrap gap-2 pt-2 border-t border-border">
                    {STATUSES.map((s) => (
                      <button key={s.value} onClick={() => setStatus(t.id, s.value)} disabled={t.status === s.value} className="px-2.5 py-1 rounded-md text-xs border border-border hover:border-neon-blue/60 disabled:opacity-40">→ {s.label}</button>
                    ))}
                  </div>
                  {t.admin_response && (
                  ...
                  <div className="flex gap-2">
                    <input value={responses[t.id] ?? ""} onChange={(e) => setResponses((r) => ({ ...r, [t.id]: e.target.value }))} placeholder="Respuesta oficial..." className="flex-1 bg-input/40 border border-border rounded-md px-3 py-2 text-sm" />
                    <button onClick={() => sendResponse(t.id)} className="px-4 py-2 rounded-md bg-gradient-neon text-primary-foreground text-sm font-semibold">Enviar</button>
                  </div>
                </div>
              );
            })}
